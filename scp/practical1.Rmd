...
**Bray-Curtis** takes into account species presence/absence, as well as abundance, whereas other measures (like Jaccard) only take into account presence/absence.
...
```{r NMDSnonRarefied}
nmds <- metaMDS(beta_dist) %>% scores() %>% as_tibble(rownames="ID")
...
```{r adonis}
test.adonis <- adonis(as.dist(beta_dist) ~ meta_nmds$status)
test.adonis
# p.value here
...
nmds.rar <- metaMDS(beta_dist.rar) %>% scores() %>% as.tibble(rownames="ID")
